...
class SwingTradingEngine:
    ...
    def execute_trade(self, signal: SwingSignal):
        ...
        if result.get("status") in ["new", "pending_new", "accepted", "pending"]:
            ...
            self.daily_trades += 1
            self.logger.info(
                f"Swing trade executed: {signal.symbol} {signal.action} {position_size} @ {signal.entry_price}"
            )
        ...
        for trade_id, trade in list(self.active_positions.items()):
            ...
            if current_data is not None and not current_data.empty:
                ...
                if (
                    ...
                    and latest["low"] <= trade["stop_loss"]
                ) or (
                    trade["order"].action == "SELL"
                    and latest["high"] >= trade["stop_loss"]
                ):
                ...
                # Check target
                if (
                    trade["order"].action == "BUY" and latest["high"] >= trade["target"]
                ) or (
    ...
    def close_trade(self, trade_id: str, reason: str = "time"):
        ...
        self.logger.info(
            f"Swing trade closed: {trade['signal'].symbol} P&L: ${profit_loss:.2f} ({reason})"
        )
