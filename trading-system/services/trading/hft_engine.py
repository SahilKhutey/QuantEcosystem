...
class HFTScalpingEngine:
    ...
    def execute_trade(self, signal: HFTSignal):
        ...
        if result.get("status") in ["new", "pending_new", "accepted", "pending"]:
            ...
            self.logger.info(
                f"Scalping trade executed: {signal.symbol} {signal.action} {position_size} @ {signal.price}"
            )
        ...
        for trade_id, trade in list(self.active_trades.items()):
            ...
            if current_data:
                if (
                    ...
                    and current_data["bid"] <= trade["stop_price"]
                ) or (
                    trade["order"].action == "SELL"
                    and current_data["ask"] >= trade["stop_price"]
                ):
    ...
    def close_trade(self, trade_id: str, reason: str = "time"):
        ...
        self.logger.info(
            f"Scalping trade closed: {trade['signal'].symbol} P&L: ${profit_loss:.2f} ({reason})"
        )
