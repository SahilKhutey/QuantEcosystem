...
class TestOrderExecutor(unittest.TestCase):
    def test_execute_trade_routing_to_alpaca(self):
        ...
        result = executor.execute_trade(
            "AAPL", 10, "buy", broker="alpaca", order_type="market", time_in_force="day"
        )

        self.assertEqual(result, {"order_id": "alpaca-123"})
        self.assertEqual(
            executor.alpaca.execute_order.call_args.kwargs.get("order_type"), "market"
        )
        self.assertEqual(
            executor.alpaca.execute_order.call_args.kwargs.get("time_in_force"), "day"
        )
        self.assertIn("limit_price", executor.alpaca.execute_order.call_args.kwargs)
        self.assertIn("stop_price", executor.alpaca.execute_order.call_args.kwargs)
        self.assertIn("client_order_id", executor.alpaca.execute_order.call_args.kwargs)
        self.assertIn("extended_hours", executor.alpaca.execute_order.call_args.kwargs)
