...
class IntradayTradingEngine:
    ...
    def _get_market_close_time(self) -> float:
        ...
        now = time.localtime()
        close_time = time.mktime(
            (now.tm_year, now.tm_mon, now.tm_mday, 16, 0, 0, 0, 0, 0)
        )
        return close_time
    ...
    def _generate_intraday_signal(self, symbol: str) -> IntradaySignal:
        ...
        bullish = (
            volume_ratio > 1.5
            and volatility > 0.002
            and current_mid > np.mean(mids[-5:])
        )
        bearish = (
            volume_ratio > 1.5
            and volatility > 0.002
        ...
        confidence = (
            0.5 + (min(0.25, volume_ratio * 0.05)) + (min(0.25, volatility * 50))
        )
    ...
    def _is_valid_signal(self, signal: IntradaySignal) -> bool:
        ...
        if (
            self.market_close_time and time.time() > self.market_close_time - 900
        ):  # 15 min before close
            return False
    ...
    def execute_trade(self, signal: IntradaySignal):
        ...
        if result.get("status") in ["new", "pending_new", "accepted", "pending"]:
            ...
            self.daily_trades += 1
            self.logger.info(
                f"Intraday trade executed: {signal.symbol} {signal.action} {position_size} @ {signal.entry_price}"
            )
    ...
    def monitor_trades(self):
        ...
        for trade_id, trade in list(self.active_positions.items()):
            ...
            if data and data["bids"] and data["asks"]:
                ...
                # Stop loss
                if (
                    trade["order"].action == "BUY" and latest_mid <= trade["stop_loss"]
                ) or (
                ...
                # Target
                if (
                    trade["order"].action == "BUY" and latest_mid >= trade["target"]
                ) or (
    ...
    def close_trade(self, trade_id: str, reason: str = "time"):
        ...
        self.logger.info(
            f"Intraday trade closed: {trade['signal'].symbol} P&L: ${pnl:.2f} ({reason})"
        )
    ...
    def get_performance_metrics(self):
        ...
        return {
            ...
            "avg_profit_per_trade": sum(t["profit_loss"] for t in self.trade_history)
            / total,
