...
class AutonomousTradingEngine:
    ...
    def start(self):
        ...
        self.threads = [
            ...
            threading.Thread(
                target=self._signal_processing_thread, name="SignalProcessing"
            ),
            threading.Thread(
                target=self._order_execution_thread, name="OrderExecution"
            ),
                ...
                for symbol in symbols:
                    try:
                        ...
                        self.logger.exception(
                            f"Error processing market data for {symbol}"
                        )
                ...
                try:
                    ...
                    if swing_signal:
                        self.signal_queue.put(
                            {"engine": "swing", "signal": swing_signal}
                        )
                    if intraday_signal:
                        self.signal_queue.put(
                            {"engine": "intraday", "signal": intraday_signal}
                        )
    ...
    def _update_performance_metrics(self):
        ...
        self.performance["total_trades"] = (
            hft_m["total_trades"] + swing_m["total_trades"] + intraday_m["total_trades"]
        )
        win_count = (
            int(hft_m["total_trades"] * hft_m["win_rate"])
