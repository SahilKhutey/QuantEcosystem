...
class TDAPI(BrokerAPI):
    ...
    def submit_order(self, order: OrderRequest) -> dict:
        ...
        try:
            ...
            if response.status_code in (200, 201):
                ...
                self.logger.info(
                    f"Order submitted: {order_data.get('order_id', 'N/A')} for {order.symbol}"
                )
                return order_data
            ...
            self.logger.error(
                f"Order submission failed: {error.get('message', 'API error')}"
            )
            return {
    ...
    def get_order_status(self, order_id: str) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error = response.json()
                self.logger.error(
                    f"Order status check failed: {error.get('message', 'API error')}"
                )
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 204:
                ...
                error = response.json()
                self.logger.error(
                    f"Order cancellation failed: {error.get('message', 'API error')}"
                )
                return False
    ...
    def get_positions(self) -> list:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error = response.json()
                self.logger.error(
                    f"Positions request failed: {error.get('message', 'API error')}"
                )
                return []
    ...
    def get_account(self) -> dict:
        ...
        try:
            response = requests.get(
                f"{self.BASE_URL}/accounts/{self.api_key}", headers=self._get_headers()
            )
            ...
            if response.status_code == 200:
                ...
                error = response.json()
                self.logger.error(
                    f"Account information request failed: {error.get('message', 'API error')}"
                )
    ...
    def get_historical_data(
        self, symbol: str, timeframe: str = "1D", start: str = None, end: str = None
    ) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error = response.json()
                self.logger.error(
                    f"Historical data request failed: {error.get('message', 'API error')}"
                )
                return {"error": "API error"}
    ...
    def get_real_time_data(self, symbol: str) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error = response.json()
                self.logger.error(
                    f"Real-time data request failed: {error.get('message', 'API error')}"
                )
                return {"error": "API error"}
